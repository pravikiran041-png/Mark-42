...
def browser_control(
    ...
    if action == "youtube":
        ...
        if not query: return "Please provide a song or query for YouTube."
    ...
    if action == "translate":
        import webbrowser, subprocess, urllib.parse
        script = '''
        ...
        try:
            ...
            if res.returncode == 0 and res.stdout.strip(): url = res.stdout.strip()
        except: pass
        if url:
            enc_url = urllib.parse.quote(url, safe='')
            webbrowser.open(f"https://translate.google.com/translate?sl=auto&tl=en&u={enc_url}")
            return "Translating the current page."
        else:
            webbrowser.open("https://translate.google.com")
            return "Opened Google Translate."

    if action == "screenshot":
        import subprocess, datetime, os, platform
        _OS = platform.system()
        ...
        if _OS == "Darwin":
            ...
            try:
                ...
                return "pyautogui is required for Windows screenshots."
