...
def toggle_bluetooth():
    if _OS == "Darwin":
        try:
            subprocess.run(["blueutil", "-p", "toggle"], capture_output=True, timeout=5)
        except Exception:
            print("[Settings] blueutil not found. Running AppleScript fallback.")
            subprocess.run(["osascript", "-e", 'tell application "System Events" to tell process "ControlCenter" to click (menu bar item 1 of menu bar 1 whose description contains "Bluetooth")'], capture_output=True, timeout=5)
    elif _OS == "Windows":
        print("[Settings] Bluetooth toggle on Windows not natively supported by this script.")
    else:
        subprocess.run(["rfkill", "toggle", "bluetooth"], capture_output=True, timeout=5)
...
def computer_settings(
    ...
    if action == "volume":
        ...
        if "mute" in desc:
            ...
            return "Volume action requires a specific command like mute or a value."
