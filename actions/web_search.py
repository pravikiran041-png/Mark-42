...
def _reporter(query: str) -> str:
    """
    Reporter Mode — Deeply researches news, background, and social media reactions.
    Outputs a comprehensive dossier for the AI anchor.
    """
    reporter_query = (
        f"Act as a master investigative reporter. Deeply research the latest news, updates, "
        f"and social media reactions regarding: {query}. You must explicitly look for verifiable posts from Instagram, X (Twitter), Facebook, or major news outlets. "
        f"Synthesize this into a highly detailed, comprehensive dossier. Conclude in a simple, easy-to-understand way. "
        f"CRITICAL RULE: You MUST explicitly name the EXACT, REAL account or page where you found the information (e.g., 'According to the official Hindustan Times Instagram page...', 'Based on a post from the @Username X page...'). "
        f"DO NOT make up or hallucinate sources. If the user goes to the page you mention, the news MUST actually be there. "
        f"VISUAL HOLOGRAMS: To show images, output the literal text `[IMG: query|url]` right before you speak about it. Pass the `url` of the news source if available, otherwise a descriptive `query`. Max 3 images per story. "
        f"Output as a detailed broadcast script."
    )
    try:
        return _gemini_search(reporter_query)
    except Exception as e:
        print(f"[WebSearch] ⚠️ Reporter Gemini grounded search failed ({e}) — falling back to synthesizing DDG...")
        raw_news_results = _ddg_news(query, max_results=10)
        formatted_news = _format_news(query, raw_news_results)
        
        fallback_query = (
            f"Act as a master investigative reporter. I am providing you with raw news data regarding: {query}. "
            f"Here is the data:\n{formatted_news}\n\n"
            f"Synthesize this into a highly detailed, comprehensive dossier. Conclude in a simple, easy-to-understand way. "
            f"CRITICAL RULE: You MUST explicitly name the EXACT source from the provided data (e.g. 'According to the Hindustan Times...'). "
            f"DO NOT hallucinate sources. "
            f"VISUAL HOLOGRAMS: To show images, output the literal text `[IMG: query|url]` right before you speak about it. Pass the `url` of the news source if available, otherwise a descriptive `query`. Max 3 images per story. "
            f"Output a detailed broadcast script."
        )
        try:
            from google import genai
            client = genai.Client(api_key=_get_api_key())
            ...
            print(f"[WebSearch] ⚠️ Gemini fallback synthesis failed ({e2})")
