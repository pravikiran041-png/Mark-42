...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  const chromaticShader = {
    uniforms: {
      ...
      uColor: { value: new THREE.Color(0xffaa30) }
    },
    vertexShader: `
      varying vec2 vUv;
      void main() {
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Push towards custom theme color dynamically
        gl_FragColor.rgb = mix(gl_FragColor.rgb, gl_FragColor.rgb * uColor * 1.3, 0.35);
      }
    `,
  };
  ...
  function setThemeColor(hexColor: string) {
    ...
    // Update color grading shader pass to match new UI color
    if (chromaticPass.uniforms.uColor) {
      chromaticPass.uniforms.uColor.value.copy(color);
    }
